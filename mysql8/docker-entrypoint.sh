...
CREATE USER 'game'@'%' IDENTIFIED WITH mysql_native_password BY '${MYSQL_ROOT_PASSWORD}';
GRANT ALL PRIVILEGES ON *.* TO 'game'@'%' WITH GRANT OPTION;

-- game@SERVER_IP - 密码固定为 uu5!^%jg
CREATE USER 'game'@'${SERVER_IP}' IDENTIFIED WITH mysql_native_password BY 'uu5!^%jg';
GRANT ALL PRIVILEGES ON *.* TO 'game'@'${SERVER_IP}';

-- game@_IP127.0.0.1 - 密码固定为 uu5!^%jg
CREATE USER 'game'@'127.0.0.1' IDENTIFIED WITH mysql_native_password BY 'uu5!^%jg';
