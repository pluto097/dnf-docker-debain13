...
创建项目文件夹：
```bash
mkdir -p dnf-docker-debain13
```

进入项目文件夹：
```bash
cd dnf-docker-debain13
```

获取`docker-compose.yml` 和 `.env` 配置文件（推荐在Compose目录下获取）：
```bash
...
#publickey.pem和privatekey.pem放入home/neople/game目录下更新
#一般情况下不需要修改docker-compose.yml文件,如需修改请自行查看源码内容
```

> 💡 **重要提示**：`publickey.pem` 和 `privatekey.pem` 均在/home/neople/game目录下。
...
部署成功后，在server/log目录下查看Logxxxxxxxx.init日志(xxxxxxxx为当天日期)：
...
- 特别说明：神迹客户端只需要放入`dnf-launcher.exe`和`Config.toml`文件即可,请勿替换`ijl15.dll`。
